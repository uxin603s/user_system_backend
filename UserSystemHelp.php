...
class UserSystemHelp{
	public static function login($success="",$cache="",$error="",$location=true){
			...
			if(mb_strlen($_GET['access_token'])==32 && preg_match("/^[a-z0-9]+$/",$_GET['access_token'])){
				...
				if($http_code!="200"){		
					...
					if(is_callable($cache)){
						...
						if($tmp=call_user_func($cache,$_GET['access_token'])){
							$result['status']=true;
							$result['data']=$tmp;
	...
	public static function success($data,$location){
		...
		Cache::set("userSystem_{$access_token}",$data,60*30);
		session_write_close();
		//data找導頁資料並導頁
		if($location){
	...
	public static function cache($access_token){
		//取得data快取return
		return Cache::get("userSystem_{$access_token}");
	}
